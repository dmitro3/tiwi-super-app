...
interface SmartMarketsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function SmartMarketsModal({ isOpen, onClose }: SmartMarketsModalProps) {
  ...
  useEffect(() => {
    ...
    const loadMarkets = async () => {
      ...
      try {
        ...
        console.error("[SmartMarketsModal] Error fetching markets:", error);
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-[#1f261e]">
            <h2 className="text-white text-xl font-semibold">Smart Markets</h2>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {isLoading ? (
              ...
              <div className="text-center py-12 text-[#7c7c7c]">
                No smart markets available
              </div>
            ) : (
              <>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {markets.map((market: SmartMarket) => (
                    <div
                      key={market.id}
                    ...
                    >
                      <div className="relative w-12 h-12 rounded-full overflow-hidden bg-[#0b0f0a]">
                        <Image
                          src={market.icon}
                          alt={market.name}
                          width={48}
                          ...
                          onError={(e) => {
                            // Fallback to local AMM icon based on market order
                            const fallbackIcon = `/assets/amms/amm-${((market.order || 1) % 7) || 1}.svg`;
                      ...
                      <span className="text-white text-sm font-medium text-center">{market.name}</span>
                    </div>
                  ))}
                </div>
