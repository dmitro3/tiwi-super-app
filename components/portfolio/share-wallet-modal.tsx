...
export default function ShareWalletModal({
  open,
  onOpenChange,
  walletAddress,
}: ShareWalletModalProps) {
  const [copied, setCopied] = useState(false);
  const [qrCopied, setQrCopied] = useState(false);

  const shareMessage = `💰 My Wallet Address:\n${walletAddress}\n\nSend tokens to this address on TIWI Protocol`;
  ...
  const handleShare = (platform: string) => {
    ...
    switch (platform) {
      ...
      case "twitter":
        ...
        break;
      case "telegram":
        shareUrl = `https://t.me/share/url?url=${encodedAddress}&text=${encodeURIComponent(`💰 My Wallet Address:\n${walletAddress}`)}`;
        break;
      default:
        return;
    }
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          link.href = url;
          link.download = `tiwi-wallet-address.png`;
          link.click();
          ...
          <div className="bg-[#0B0F0A] border border-white/5 rounded-lg p-4">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-[#010501] border border-white/5 rounded-lg px-4 py-2 overflow-hidden">
                <p className="font-manrope text-xs text-white break-all">
                  {walletAddress}
                </p>
              </div>
              <button
